...
LightAgentGateway::Method toGatewayMethod(HttpMethod method) {
  switch (method) {
    case METHOD_GET:
      return LightAgentGateway::Method::Get;
    case METHOD_POST:
      return LightAgentGateway::Method::Post;
    case METHOD_HEAD:
      return LightAgentGateway::Method::Head;
    default:
      return LightAgentGateway::Method::Unsupported;
  }
}
...
void HttpData::handleRead() {
  ...
  do {
    ...
    if (state_ == STATE_RECV_BODY) {
      optional<int> content_length = getContentLength();
      if (!content_length) {
...
HeaderState HttpData::parseHeaders() {
  ...
  if (hState_ == H_END_LF) {
    if (body_start == string::npos || body_start > str.size())
      str.clear();
    else
      str = str.substr(body_start);
...
optional<int> HttpData::getContentLength() const {
  for (const auto &[key, value] : headers_) {
    if (toLowerCopy(key) != "content-length") continue;
    try {
      int length = stoi(value);
      if (length < 0) return nullopt;
      return length;
    } catch (...) {
      return nullopt;
    }
...
bool HttpData::isKeepAliveRequested() const {
  for (const auto &[key, value] : headers_) {
    if (toLowerCopy(key) != "connection") continue;
    string loweredValue = toLowerCopy(value);
    return loweredValue == "keep-alive";
  }
  return false;
}

AnalysisState HttpData::analysisApiRequest() {
  const string apiPath = "/" + fileName_;
  string_view body;
  optional<int> contentLength;
  if (method_ == METHOD_POST) {
    contentLength = getContentLength();
    if (!contentLength || static_cast<int>(inBuffer_.size()) < *contentLength) {
      return ANALYSIS_ERROR;
    }
    body = string_view(inBuffer_.data(), static_cast<size_t>(*contentLength));
  }

  LightAgentGateway::Request gatewayRequest{toGatewayMethod(method_), apiPath,
                                            body};
  optional<LightAgentGateway::Response> gatewayResponse =
      LightAgentGateway::handle(gatewayRequest);
  if (!gatewayResponse) {
    return ANALYSIS_ERROR;
  }

  string header;
  header += "HTTP/1.1 " + to_string(gatewayResponse->statusCode) + " " +
            gatewayResponse->reason + "\r\n";
  header += "Content-Type: " + gatewayResponse->contentType + "\r\n";
  header += "Content-Length: " + to_string(gatewayResponse->body.size()) +
            "\r\n";
  header += "Server: LightAgent Gateway\r\n";
  if (isKeepAliveRequested()) {
    keepAlive_ = true;
  ...
  if (method_ != METHOD_HEAD) {
    outBuffer_ += gatewayResponse->body;
  }
  if (contentLength) {
    inBuffer_ = inBuffer_.substr(static_cast<size_t>(*contentLength));
  }
  return ANALYSIS_SUCCESS;
}

AnalysisState HttpData::analysisRequest() {
  if (fileName_.size() >= 4 && fileName_.compare(0, 4, "api/") == 0) {
    return analysisApiRequest();
  }
